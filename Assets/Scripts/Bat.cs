...
namespace Metroknight
{
    public class Bat : Enemy
    {
        ...
        protected override void UpdateEnemyStates()
            {
                ...
                switch (GetCurrentEnemyState)
                {
                    ...
                    case EnemyStates.Bat_Chase:
                        if (soundTimer == 3f)
                        {
                            audioSource.PlayOneShot(BatSound, .02f);
                            StartCoroutine(ResetSoundTimer());
                        }
        ...
        // Override the EnemyHit method to handle damage and state changes
